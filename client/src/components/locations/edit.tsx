...
const LocationEdit: React.FC = () => {
    ...
    const save = async () => {
        ...
        requestBody.append('_id', locationInfo._id)
        requestBody.append('location', formData.locationName)
        formData.file && requestBody.append('file', formData.file, formData.locationImageName)
    ...
    useEffect(() => {
        const load = async () => {
            const location: ILocation = await dataApi.location(id)

            setLocationInfo({ ...location })
            setImagePreview(dataApi.locationImageUrl(location.location))
        }

        if (locationInfo) {
            const existingFormData: ILocationForm = {
                locationName: locationInfo.location,
                locationImageName: locationInfo.imageFileName,
                file: null,
            }
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                    <div className="lookup-form mt-4">
                        <div className="control-group">
                            ...
                            <input name="locationName" className="text-input" onChange={handleChange} value={formData?.locationName} />
                        </div>
                        <div className="control-group">
